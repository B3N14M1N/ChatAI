...
class ChatPipeline:
    ...
    async def _get_compact_ctx_cached(
        self, conversation_id: int, need: str
    ) -> List[dict]:
        """
        'none' => 0 messages
    ...
    async def handle_user_message(
        ...
        *,
        ...
        if len(user_text) > 400:
            summary_text, summ_usage = self.oa.summarize(user_text)
        # user message create
        ...
        compact_ctx = await self._get_compact_ctx_cached(
            conversation_id, intent.context_need
        )
